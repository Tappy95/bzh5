<template>
    <div id="app" style="height: 100vh;background: #8A16E4;overflow: auto;">
        <div class="zb_head">
        ...
        <div class="zb_tb">
            ...
            <p style="margin-left:-0.9rem;">排名</p>
...
export default {
    ...
    methods:{
        ...
        getInfo(index){
            ...
            this.$get('/api/rankPig/listFDay',paramterData).then(res => {
                if((res.statusCode+"").startsWith("2")){
                    if((res.statusCode+"").startsWith("2")){
                        for(var i = 0;res.data.list.length>i;i++){
                            res.data.list[i].pigBalance = (res.data.list[i].pigBalance / 10000).toFixed(2);
                            if(res.data.list[i].rewardAmount == '0' || res.data.list[i].rewardAmount == 0){
                                res.data.list[i].rewardAmount = '待派奖';
                            }else{
                                res.data.list[i].rewardAmount = (res.data.list[i].rewardAmount / 10000).toFixed(2)+'万';
                            }
                        }
