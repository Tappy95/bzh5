    ...
    <div id="app" style="height: 100vh;background: #F3F5F8;overflow: auto;">
        <div class="against-bg">
            ...
            <div class="against-content">
                <p>500-5000金币入场</p>
                <div><input type="text" v-model="input" style="border: none;background-color: inherit;outline:none;"></div>
                <p>我的账户：{{coin}}金币</p>
                <div>
                    <img src="../../assets/submit.png" alt="" @click="invite">
                </div>
        ...
        invite(){
           ...
           if(parseInt(this.input) > parseInt(this.coin)){
            ...
            let parameterData = {
                token:this.token,
                imei:this.imei,
                defenseCoin:this.input,
                entryCode:this.entryCode
            }
            this.$get('/api/mFightingInfo/joinRoom',parameterData).then(res => {
                if((res.statusCode+"").startsWith("2")){

                  window.location.href = "/friendWait.html?token="+this.token+'&imei='+this.imei+
                                         '&enemyimg='+res.data.enemyImg+'&img='+res.data.img+'&enemyMoile='+ res.data.enemyMoile+
                                         '&role='+res.data.role+'&mobile='+res.data.mobile+'&entryCode='+res.data.entryCode+
                                         '&quesNum='+res.data.quesNum+'&fightId='+res.data.fightId;
