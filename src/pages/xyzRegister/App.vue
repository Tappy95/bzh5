             ...
             <div class="message_area">
               <div id="con1" ref="con1" :class="{anim:animate==true}">
                 <div class="item" v-for="(item,index) in scrollData">
                   ...
                   <span class="c_1">恭喜{{item.aliasName}}</span>
                   <span class="c_2">获得了{{item.goodsName}}</span>
                 </div>
               </div>
             </div>
...
<script type="text/javascript">
  ...
  export default {
    ...
    methods: {
      ...
      getScroll(){
        this.$get('/wish/guessSpecialGoods/newsH5').then(res => {
        ...
        this.$get('/push/validateCode/validateSmsCode', parameterData).then(res => {
          if((res.statusCode+"").startsWith("2")){
            if(res.data.res==true){
              ...
              this.$get('/api/userInfo/regWishH5', parameter).then(res => {
...
<style scoped>
  *{
  ...
  .bg{
    ...
    width: 100%;
    height: 38.75rem;
    background: url('../../assets/xyxRBg.jpg') no-repeat;
