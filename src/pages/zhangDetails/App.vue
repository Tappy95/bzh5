        ...
        <scroller :on-refresh="refresh" :on-infinite="infinite" :noDataText="noDataText">
            ...
            <div class="wr_list" v-for="(item,index) in listData" v-if="listData.length>0">
                ...
                <div class="wc_list_right">
                    <p  v-if="item.changedType == '提现成功'" style="color:#01CF40;">
                        <!-- <span>{{item.flowType}}</span> -->
                        -{{item.amount / 11000 | keepTwoNum}}元
                    </p>
                    <p v-else>+{{item.amount / 11000 | keepTwoNum}}元</p>
                    <p>{{item.coinBalance / 11000 | keepTwoNum}}</p>
                </div>
            </div>
            ...
            this.$get('/api/lCoinChange/list',parameterData).then(res => {
                if((res.statusCode+"").startsWith("2")){
                    for(var i = 0;i<res.data.list.length;i++){
                        if(res.data.list[i].changedType == '3'){
                           res.data.list[i].changedType = '提现成功'
                           ...
                           res.data.list[i].changedType = '注册奖励'
                        }else{
                           res.data.list[i].changedType = '心愿宝箱抽奖'
                        }
